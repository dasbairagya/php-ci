...
class Crud_model extends CI_Model {
    ...
    function create_project() {
        //$data['title'] = $this->input->post('title');
        $data['service_id'] = $this->input->post('add_service_id');
        $data['tasks'] = $this->input->post('tasks');
        $data['add_extra_task'] = $this->input->post('add_extra_task');
        ...
        // if(isset($this->input->post('add_extra_task'))){
        //     $data['service_id'] = $this->input->post('add_service_id');
        //     $data['tasks'] = $this->input->post('tasks');
        //     $data['add_extra_task'] = $this->input->post('add_extra_task');
        //      addExtraField($data);
        ...
        // function addExtraField($data){
        //     $data['add_extra_task'] = $this->input->post('add_extra_task');  
        //     $addservices = array('tasks' => serialize($data['add_extra_task']), );
        //     $this->db->insert('add_service', $addservices);
        //     $this->db->where(array('service_id' =>$data['service_id']);
        // }


     ////////projects/////////////
    function save_service() {
       $data['add_service_id']=$this->input->post('add_service_id');
       //$data['title'] = $this->input->post('title');
       $data['project_code'] = substr(md5(rand(100000000, 200000000)), 0, 10);
      // $data['description'] = $this->input->post('description');
        ...
        $this->db->insert('create_project', $data);
        $project_code = $this->db->get_where('create_project', array('service_id' => $this->db->insert_id()))->row()->project_code;
	...
	function create_project1() {
        ...
        $this->db->insert('saveservice', $data);
        $project_code = $this->db->get_where('saveservice', array('project_id' => $this->db->insert_id()))->row()->project_code;
    ...
    function create_service() {
         ...
         $data['advanceper'] = $this->input->post('advanceper');

         $data['docs'] = serialize($this->input->post('docs'));
        

         $data['docrequired'] = serialize($this->input->post('docrequired'));
          // $data['docrequired'] = serialize($this->input->post('pancard'));
          // $data['docrequired'] = serialize($this->input->post('dl'));
          // $data['docrequired'] = serialize($this->input->post('Adharcard'));
          // $data['docrequired'] = serialize($this->input->post('voterid'));

         $data['tasks'] = serialize($this->input->post('tasks'));

        
        

        $this->db->insert('add_service', $data);
        $project_code = $this->db->get_where('add_service', array('service_id' => $this->db->insert_id()))->row()->project_code;
    ...
    //@Auther Vikram prajapat
    // date 25/11/16 
    function create_project_task($project_code = '') {
        $data['title'] =$this->input->post('tasks');
        $data['description'] = $this->input->post('description');
    ...
    function download_project_file($project_file_id = '') {
     ...
     //@auther Vikram Singh
    //date 26//11/16
     function download_req_document($project_id = '') {
        $file_name = $this->db->get_where('project_file', array('project_id' => $project_id))->row()->name;
    ...
    function download_project_document($project_file_id = '') {
        $file_name = $this->db->get_where('project_file', array('project_file_id' => $project_file_id))->row()->name;
        $this->load->helper('download');
        $data = file_get_contents("uploads/team_task_)file/" . $file_name);
        $name = $file_name;
        force_download($name, $data);
    }
...
//add_task
    ...
    function calendar_event_delete($calendar_event_id = '') {
     ...
     ///// CALENDAR reminder  //////
    function calendar_reminder_add() {
        $data['title'] = $this->input->post('title');
        $data['description'] = $this->input->post('description');
        $data['colour'] = $this->input->post('colour');
        $data['start_timestamp'] = strtotime($this->input->post('start_timestamp'));
        ...
        $data['user_type'] = $this->session->userdata('login_type');
        $data['user_id'] = $this->session->userdata('login_user_id');
        $this->db->insert('calendar_reminder', $data);
    }


      function calendar_reminder_edit($calendar_reminder_id = '') {
        $data['title'] = $this->input->post('title');
        $data['description'] = $this->input->post('description');
        $data['colour'] = $this->input->post('colour');
        $data['start_timestamp'] = strtotime($this->input->post('start_timestamp'));
       // $data['end_timestamp'] = strtotime($this->input->post('end_timestamp'));
        $this->db->where('calendar_reminder_id', $calendar_reminder_id);
        $this->db->update('calendar_reminder', $data);
    }

    function calendar_reminder_delete($calendar_reminder_id = '') {
        $this->db->where('calendar_reminder_id', $calendar_reminder_id);
        $this->db->delete('calendar_reminder');
    }
