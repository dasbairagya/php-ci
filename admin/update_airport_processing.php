...
if(isset($_GET['status_id']))
	...
	try{	
		$status_id=$_GET['status_id'];
         ...
         $stmt = $conn->prepare("UPDATE ride_to_airport SET status='completed'  WHERE id=$status_id ");
         $stmt -> execute();

         $stmt1 = $conn->prepare ("SELECT driver_name FROM ride_to_airport WHERE id='".$status_id."'");
         $stmt1 -> execute();
